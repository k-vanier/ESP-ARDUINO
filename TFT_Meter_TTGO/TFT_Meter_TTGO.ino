...
void loop() {
  if (updateTime <= millis()) {
    updateTime = millis() + 5; // Update meter every 35 milliseconds
...
void analogMeter()
{
  ...
  tft.drawString("STRING", M_SIZE*(3 + 230 - 40), M_SIZE*(119 - 20), 2); // Units at bottom right
  tft.drawCentreString("VALEUR", M_SIZE*120, M_SIZE*75, 4); // Comment out to avoid font 4
